...
import type { HousingContract } from '../utils/types';
import { formatCurrency } from '../utils/formatters';
import { XIcon } from './Icons';

interface Props {
  isOpen?: boolean;
  contracts: HousingContract[];
  municipalityName: string;
  onClose: () => void;
  isLoading: boolean;
}

const HousingContractsModal: React.FC<Props> = ({
  isOpen = true,
  contracts,
  municipalityName,
  onClose,
  ...
  return (
    <div className="modal-backdrop" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="housing-title">
      <div className="modal-content max-w-4xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <div>
            <h2 id="housing-title" className="text-lg font-bold text-slate-800">Contratos de Vivienda</h2>
        ...
        <div className="p-4">
          ...
          {!isLoading && contracts.length === 0 && (
            <div className="text-center py-12 text-slate-500 text-sm">No se encontraron contratos de vivienda.</div>
          )}
          {!isLoading && contracts.length > 0 && (
                  ...
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        {c.contractorName} • {c.subsidyType} • {c.beneficiaries} beneficiarios
                      </p>
                    </div>
...
export default React.memo(HousingContractsModal);
